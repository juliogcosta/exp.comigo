...
import br.com.comigo.atendimento.api.application.usecase.ClienteUseCases;
...
import br.com.comigo.atendimento.api.domain.repository.ClienteRepository;
...
import br.com.comigo.atendimento.api.mapper.ClienteMapper;
...
public class ClienteServiceImpl implements ClienteUseCases {
    ...
    public ClienteDTO getClienteDetailsById(Long id) {
        Cliente cliente = this.clienteRepository.findById(id).orElseThrow(() -> new RuntimeException("Cliente not found"));
        return this.clienteMapper.toDto(cliente);
    }

    @Override
    public ClienteDTO getClienteDetailsByCpf(Cpf cpf) {
        Cliente cliente = this.clienteRepository.findByCpf(cpf).orElseThrow(() -> new RuntimeException("Cliente not found"));
        return this.clienteMapper.toDto(cliente);
    }
    ...
    public void addVeiculoToCliente(VeiculoDTO veiculoDTO, Long clienteId) {
        ...
        this.veiculoRepository.create(veiculo);
    }
