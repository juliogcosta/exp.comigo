...
import br.com.comigo.atendimento.adapter.util.JpaEndereco;
import br.com.comigo.atendimento.adapter.util.JpaTelefone;
import br.com.comigo.atendimento.domain.aggregate.atendimento.Atendimento;
import br.com.comigo.atendimento.domain.aggregate.atendimento.repository.AtendimentoRepository;
import br.com.comigo.atendimento.domain.util.StatusDeAtendimento;
import br.com.comigo.atendimento.mapper.aggregate.atendimento.AtendimentoMapper;
...
public class AtendimentoRepositoryImpl implements AtendimentoRepository {
    ...
    public void update(Atendimento atendimento) {
        ...
        jpaAtendimento.setClienteTelefone(
                new JpaTelefone(atendimento.getClienteTelefone().numero(), atendimento.getClienteTelefone().tipo()));
        jpaAtendimento.setClienteWhatsapp(
                new JpaTelefone(atendimento.getClienteWhatsapp().numero(), atendimento.getClienteWhatsapp().tipo()));
        ...
        jpaAtendimento.setBase(new JpaEndereco(atendimento.getBase().logradouro(), atendimento.getBase().numero(),
                atendimento.getBase().complemento(), atendimento.getBase().bairro(), atendimento.getBase().cidade(),
                atendimento.getBase().estado(), atendimento.getBase().cep()));
        jpaAtendimento.setOrigem(new JpaEndereco(atendimento.getOrigem().logradouro(), atendimento.getOrigem().numero(),
                atendimento.getOrigem().complemento(), atendimento.getOrigem().bairro(),
                atendimento.getOrigem().cidade(), atendimento.getOrigem().estado(), atendimento.getOrigem().cep()));
        jpaAtendimento.setDestino(new JpaEndereco(atendimento.getDestino().logradouro(),
                atendimento.getDestino().numero(), atendimento.getDestino().complemento(),
                atendimento.getDestino().bairro(), atendimento.getDestino().cidade(), atendimento.getDestino().estado(),
    ...
    public List<Atendimento> searchByStatus(StatusDeAtendimento status) {
        return this.jpaAtendimentoRepository.findByStatus(status.name()).stream()
                .map(atendimentoMapper::fromJpaToDomain).collect(Collectors.toList());
    }
